...
print STDERR " both depth >= $min_depth and input should be remove-dupl results subset \n\n\n";
 ...
 my $usage = "$0 \n <WT_isMeth> <WT_pre> <mut_isMeth> <mut_pre>  <outdir> <postfix>\n\n";
...
my $output = File::Spec->catfile($outdir,$WT_pre ."_vs_". $mut_pre . "_diff_bases_P" . $p_cutoff . "_AllMinDep" . $min_depth . "_" . $postfix . ".txt");
