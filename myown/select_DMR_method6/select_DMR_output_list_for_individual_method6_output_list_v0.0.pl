...
#method 6
#v0.0
#use the score not present/absent of the wig file
#sum of score in mutant/ sum of score in control >= 2 and 
#sum of score in mutant >=  cutoff
#or 
#sum of score in control == 0 and sum of score in mutant >= zore_cutoff
...
#my $usage = "$0 \n  <control_input> <control_pre> <input>  <input_pre> <winSize> <sliding_bp> <merged_gap_cutoff> <big_score_cutoff> <fold_change> <outdir> \n\n";
my $usage = "$0 \n <winSize> <sliding_bp> <merged_gap_cutoff> <big_score_cutoff> <fold_change> <outdir>  <control_input> <control_pre> <input>  <input_pre> \n\n";
...
my $postfix = "_method6_win" . $window_size . "_sliding" . $sliding_bp . "_gap" . $merged_gap_cutoff . "_Score" . $big_score_cutoff . "_fold" . $fold_change_cutoff . "_zero_score_cutoff" . $zero_score_cutoff .".txt";
...
select_region5( $window_size, $sliding_bp, $merged_gap_cutoff, $big_score_cutoff, $fold_change_cutoff, \@line_hyper,  \%pos_mut,  \%den_mut,  \%pos_wt,   \%den_wt) ;
#hypo in wt
...
select_region5( $window_size, $sliding_bp, $merged_gap_cutoff, $big_score_cutoff, $fold_change_cutoff, \@line_hypo,   \%pos_wt,   \%den_wt,   \%pos_mut,  \%den_mut);			  
...
sub output_hyper5{
	...
	print OUT join("\t", ("chr", "start", "end", "mut_score", "wt_score", "fold_change")), "\n";
...
sub output_hypo5{
	...
	print OUT join("\t", ("chr", "start", "end", "wt_score", "mut_score", "fold_change")), "\n";
...
sub select_region5{
	...
	#foreach my $chr (@chr_array)
		...
		#	my $unit_percentage = $hypo_zero_label ;
			my $unit_fold_change = $hypo_zero_label ;
			
			
			if($score_hypo > 0) {
			#	$unit_percentage = sprintf("%.2f", 100 * ( $score_hyper - $score_hypo  ) / $score_hypo );
				$unit_fold_change = sprintf("%.3f",  $score_hyper / $score_hypo );
			}
		...
		#		 ($unit_percentage == $hypo_zero_label and $score_hyper >= $zero_score_cutoff)){
			if ( ($score_hyper >= $big_score_cutoff_sub and $unit_fold_change >= $percentage_cutoff_sub) or 
				 ($unit_fold_change == $hypo_zero_label and $score_hyper >= $zero_score_cutoff)){
				...
				#	my $final_percentage = $hypo_zero_label;
					my $final_fold_change = $hypo_zero_label;
					if($score_hypo_total > 0 ){
						$final_fold_change = sprintf("%.3f", $score_hyper_total / $score_hypo_total);
					}
					
					if( ( $score_hyper_total >= $big_score_cutoff_sub and $final_fold_change >=  $percentage_cutoff_sub) 
							or ( $final_fold_change == $hypo_zero_label and  $score_hyper_total >= $zero_score_cutoff) ){
								
					#		if($final_percentage == $hypo_zero_label) {$final_percentage = 10000}
							if($final_fold_change == $hypo_zero_label) {$final_fold_change = 10000}
							push (@{$ref_line}, [$last_chr, $last_start, $last_end, $score_hyper_total, $score_hypo_total, $final_fold_change]);
