...
##########################
=head
my $cmd_uzip1 = "time zcat $indir/*_1.fq.gz > $fq_in1";
...
my $cmd_uzip2 = "time zcat $indir/*_2.fq.gz > $fq_in2";
...
#PE reads 1,2
my $in_P1 = File::Spec->catfile($indir, $inpre."_reads1.txt");
my $in_P2 = File::Spec->catfile($indir, $inpre."_reads2.txt");

#SE mates 1,2
my $in_S1 = File::Spec->catfile($indir, $inpre."_mates1.txt");
my $in_S2 = File::Spec->catfile($indir, $inpre."_mates2.txt");
...
my $log = File::Spec->catfile($outdir, $outpre."_brat_bw_log.txt");
...
my $cmd_acgt_count = "time $acgt_count -r $fas_ref -P $outdir/$outpre -p $pair_list -s $single_list -B";
