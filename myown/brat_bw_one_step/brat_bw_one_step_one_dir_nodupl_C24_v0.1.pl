...
my $ref = "/Users/tang58/DataBase/BRAT_reference_files/C24_brat_bw_index";
die unless (-d $ref);

#my $fas_ref = "/Users/tang58/DataBase/BRAT_reference_files/Col0/Col0_references_names.txt";
my $fas_ref = "/Users/tang58/DataBase/BRAT_reference_files/C24_brat_bw_index/C24_references_names.txt";
...
my ( $tar_gz ) = grep /\.tar\.gz$/, readdir DIR;
closedir DIR;
...
my $cmd_tar = "time tar -xzvf $tar_gz";
...
if(!$debug){
	...
	opendir(DIR, $indir) or die "dir";
	my ($name1, $name2) = grep /sequence\.txt$/, readdir DIR;
	closedir DIR;
	
	$fq_in1 = File::Spec->catfile($indir, $name1);
	$fq_in2 = File::Spec->catfile($indir, $name2);
	
	die unless ( $fq_in1 =~ /_1_sequence.txt$/);
	die unless ( $fq_in2 =~ /_2_sequence.txt$/);
}
...
my $rm_cmd = "rm -f $indir/*_sequence.txt $indir/*fastq";
