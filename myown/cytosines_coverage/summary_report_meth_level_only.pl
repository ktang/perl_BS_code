...
my @files = grep /_info_DepthCutoff\.txt$/, readdir (DIR);
closedir(DIR);
...
print OUT join("\t", ("sample", "CG_meth_level", "CHG_meth_level", "CHH_meth_level", "total_meth_level")), "\n";
...
#	print OUT join("\t", ($pre,$avg_dep, $dep1, $dep2, $cg, $chg, $chh, $meth_level, $error )), "\n";
	print OUT join("\t", ($pre, $cg, $chg, $chh, $meth_level )), "\n";
}
