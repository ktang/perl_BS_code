#!/usr/bin/perl -w
 ...
 # v0.1 is mainly used for batch mode
 # to cal methylaiton level of the selected DMR
 
 # v0.2 is for single isMeth file
...
my $usage = "$0 \n <isMeht_file> <label> <bed_like_file> <outdir> <depth>\n\n";
...
#	my ( %mC_list_low,  %dep_list_low,  @percentage_list_low );
	...
	read_isMeth_get_mC_dep_single( $isMeth_file, \%pos,
								    \%mC_list,  \%dep_list,  \@percentage_list, 
									\%C_num_stat, $depth_cutoff);
...
sub read_isMeth_get_mC_dep_single{
	my ($high_file,  $region_in_list_ref, 
	...
	#	if($dep_high >=  $depth_cutoff_sub and $dep_low >= $depth_cutoff_sub ){
		if($dep_high >=  $depth_cutoff_sub ){
...
sub output_list{
	...
	print OUT join("\t", ($head,  "informative_C_num", "CG", "CHG", "CHH", 
								$label_ref . "_CG",  $label_ref . "_CG_per",  $label_ref . "_CHG", 
								$label_ref .  "_CHG_per", $label_ref .  "_CHH", $label_ref . "_CHH_per", 
								$label_ref .   "_C_per",  $label_ref .   "_MethLevel_perC")), "\n";
	...
	#	my $low_C_per =  sprintf("%.4f",  100 * $low_mC_total/$low_dep_total);	
		...
		my ($high_C_per, $low_C_per) = ( "NA", "NA" );
		if( $high_dep_total != 0 ){
			$high_C_per = sprintf("%.4f",  100 * $high_mC_total/$high_dep_total);
		}
...
sub	output_hyper_list{
	...
	foreach my $i (1..$last_index){
		...
		if( $low_dep_total != 0 ){
			$low_C_per =  sprintf("%.4f",  100 * $low_mC_total/$low_dep_total);
		}
...
#							\%mC_list_high, \%dep_list_high, \@percentage_list_high,
#							\%mC_list_low,  \%dep_list_low,  \@percentage_list_low ,   $depth_cutoff);
