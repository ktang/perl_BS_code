...
my $postfix = "_method5_win" . $window_size . "_sliding" . $sliding_bp . "_gap" . $merged_gap_cutoff . "_netDMC" . $big_score_cutoff . "_per" . $percentage_cutoff .".txt";
...
sub select_region5{
	...
	#foreach my $chr (@chr_array)
		...
		foreach my $i(0..(int( ($len - 1) / $sliding_bp_sub ) ) ){
			...
			if($score_hypo > 0) {
				$unit_percentage = sprintf("%.2f", $net_score / $score_hypo * 100);
			}
			if ( $score_hyper >= $big_score_cutoff_sub and $unit_percentage >= $percentage_cutoff_sub){
				...
				if($chr ne $last_chr || $narrow_donw_win_start > $last_end + $merged_gap_cutoff_sub + 1){ # not overlap
					...
					if($score_hypo_total >0){
						$final_percentage = sprintf("%.2f", ($score_hyper_total - $score_hypo_total) / $score_hypo_total * 100);
					}
