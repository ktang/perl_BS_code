...
#v0.2 
#modified from v0.1:
#only those pos with depth >= cutoff in both samples will be considered
...
my $usage = "$0 \n <depth_cutoff> <control_pre> <control_input> <input> <outdir> <pre> <winSize> <sliding_bp> <merged_gap_cutoff> <big_score_cutoff> <increase_percentage_cutoff>\n\n";
die $usage unless (@ARGV == 11);

my $dep_cutoff		  = shift or die "dep_cutoff";
...
my $postfix = "_method5.2BothDep_" . $dep_cutoff . "_win" . $window_size . "_sliding" . $sliding_bp . "_gap" . $merged_gap_cutoff . "_netDMC" . $big_score_cutoff . "_per" . $percentage_cutoff . "_zero_score_cutoff3.txt";
...
read_isMet($colA_wig, $input, \%pos_wt, \%pos_mut, $dep_cutoff);
...
#read_isMet($colA_wig, $input, \%pos_wt, \%pos_mut);
sub read_isMet{
	my ($isMet_ctrl, $isMet_mut, $ref_pos_wt, $ref_pos_mut, $dep_cutoff_sub) = @_;
	
	open(IN1, $isMet_ctrl) or die;
	open(IN2, $isMet_mut) or die;
...
#chr1    2       +       CHH     0       0       0       0
	...
	if ($dep1 >= $dep_cutoff_sub and $dep2 >= $dep_cutoff_sub ){
		#	my $type = $a1[3];
			my ($chr, $pos) = @a1[0..1];
			if($a1[-1] == 1 ){
				$ref_pos_wt->{$chr}->{$pos}  = abs($a1[6]);
			}
			
			if($a2[-1] == 1 ){
				$ref_pos_mut->{$chr}->{$pos} = abs($a2[6]);
			}
