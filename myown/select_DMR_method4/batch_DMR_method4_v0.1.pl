...
my $usage = "$0 <ctrl_wig> <ctrl_label>  <indir> <outdir>";
die $usage unless(@ARGV == 4);


my $control_wig = shift or die "control_wig";
my $ctrl_label = shift or die "ctrl_label";

my $indir = shift or die;
my $outdir = shift or die;
die unless (-d $indir);
die unless (-d $outdir);die unless (-e $control_wig);
