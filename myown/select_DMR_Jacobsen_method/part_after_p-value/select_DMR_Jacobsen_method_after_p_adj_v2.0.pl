...
# v2.0 
# only select based on adj p-value, merge tiling regions
# no 10 DMC and 2fold change cutoff
#adjust regions as first and last cytosines
...
my $usage = "$0 \n <p-adj_file> <WT_isMeth> <mut_isMeth> <outdir> <outpre> <p-value_cutoff>\n\n";
...
my $p_value_cutoff = shift or die;
...
my $output_hyper = File::Spec->catfile($outdir, $outpre."_hyper_JacobsenMethod_Bin". $bin_size . "_sliding" . $sliding_size . "_gap". $allowed_gap ."_BH_adj_p". $p_value_cutoff . "_depth". $depth_cutoff. "_v2.0.txt");
my $output_hypo  = File::Spec->catfile($outdir, $outpre."_hypo_JacobsenMethod_Bin".  $bin_size . "_sliding" . $sliding_size . "_gap". $allowed_gap ."_BH_adj_p". $p_value_cutoff . "_depth". $depth_cutoff. "_v2.0.txt");
...
my @raw_list_hyper; #push @raw_list_hyper, join("_", ( $chr, $start, $end )) ;
my @raw_list_hypo;
...
#@hyper_list_border = @list_hyper_merged;

#record_region_pos(\@hyper_list_border, \%region_in_hyper);
record_region_pos(\@list_hyper_merged, \%region_in_hyper);
...
read_isMeth_get_mC_dep_2($mut_in, $wt_in, \%region_in_hyper, \%mC_list_hyper, \%dep_list_hyper, \%percentage_list_hyper, \%C_number_hyper, \@list_hyper_merged);
#adjust list borderary in read_isMeth_get_mC_dep_2 to last and first cytosine
print STDERR "Done\n";
...
last_filter2(\@list_hyper_merged, \%mC_list_hyper, \%dep_list_hyper, \%percentage_list_hyper, \@final_list_hyper, \%C_number_hyper);
...
#@hypo_list_border = @list_hypo_merged;

record_region_pos(\@list_hypo_merged, \%region_in_hypo);
...
read_isMeth_get_mC_dep_2($wt_in, $mut_in, \%region_in_hypo, \%mC_list_hypo, \%dep_list_hypo, \%percentage_list_hypo, \%C_number_hypo, \@list_hypo_merged);
print STDERR "Done\n";
...
last_filter2(\@list_hypo_merged, \%mC_list_hypo, \%dep_list_hypo, \%percentage_list_hypo, \@final_list_hypo , \%C_number_hypo);
print STDERR "output...\n";
...
sub output_hyper_list{
	...
	print OUT join("\t", ( "chr", "start", "end" , "CG_num", "CHG_num", "CHH_num", 
...
sub last_filter2 {
	...
	#	my ($chr, $start, $end, $num) = split "_", $list_ref->[$i];
		my ($chr, $start, $end) = split "_", $list_ref->[$i];
	...
	#	push @{$final_list_ref}, [$chr, $start, $end, $num, $cg_num, $chg_num, $chh_num,
		push @{$final_list_ref}, [$chr, $start, $end,       $cg_num, $chg_num, $chh_num,
...
#read_isMeth_get_mC_dep_2($wt_in, $mut_in, \%region_in_hypo, \%mC_list_hypo, \%dep_list_hypo, \%percentage_list_hypo, \%C_number_hypo, \@hypo_list_border);


sub read_isMeth_get_mC_dep_2{
	my ($hyper_file, $hypo_file, $region_in_list_ref, 
	$mC_list_ref, $dep_list_ref, $percentage_list_ref, $C_num_ref,
	$list_border_sub) = @_;
	...
	my $last_index = scalar(@{$list_border_sub}) - 1;
	foreach my $i(0..$last_index){
		
		my @temp_sorted_array = sort {$a <=> $b} @{$temp_array[$i]};
		
		my $last_item_index = scalar(@{$temp_array[$i]}) - 1 ;
	#	my $start = $temp_array[$i]->[0];
	#	my $end   = $temp_array[$i]->[$last_item_index];
		
		my $start = $temp_sorted_array[0];
		my $end   = $temp_sorted_array[$last_item_index];
		
		my ($chr, $s, $e) = split "_",  $list_border_sub->[$i];
		$list_border_sub->[$i] = join("_", ($chr, $start, $end));
	}
...
#	my @real_border_list ;
	...
	while ($l_h =  <HYPER>, $l_l = <HYPO>){
		...
		push @{$real_border_list[$index]}, $pos;
