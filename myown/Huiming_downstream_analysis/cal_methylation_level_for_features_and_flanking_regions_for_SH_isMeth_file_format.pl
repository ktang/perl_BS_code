...
#	/Volumes/Macintosh_HD_2/idm1_new_met_data/UCR_server_data/Lane4_JKZ3_ros1_4/s_4_isMeth.txt
#	/Volumes/Macintosh_HD_2/idm1_new_met_data/UCR_server_data/Lane5_JKZ4_ros1_4/s_5_isMeth.txt

#my  $usage = "$0 <db_label> <input_db> <feature_bin_num> <flaking_bp> <flaking_bin_num> <input_list> <outdir> <outpre>";
my  $usage = "$0 \n\n<feature_bin_num> <flaking_bp> <flaking_bin_num>  <isMeth_file> <bed_region_list> <outdir> <outpre>\n\n";
...
my $output = File::Spec->catfile($outdir, $outpre . "_binNum" . $feature_bin_num . "_flaking" . $flaking_bp . "bp_binNum" . $flaking_bin_num . ".txt");
...
read_list3($input_list_file, \@input_list);
...
#chr1	6587128	6588246	1119	AT1TE21355	-	ATREP10A	RC/Helitron	AT1G19086	6588542	6589338	+	295
...
#	}
	while(<IN>){
		...
		my @a = split "\t";
		$a[0] = lc($a[0]);
#		$ref->[$i] = join("_", @a[0..3]);## chr start end strand
		$ref->[$i] = join("_", (@a[0..2], $a[5]) );
	}
	close(IN);
}
