...
#v0.1 include error rate in file name
...
my $cutoff_file = File::Spec->catfile($outdir,$pre. "_ErrorRate_"  . $rate . "_separately_called_cutoff.txt");
my $output = 	  File::Spec->catfile($outdir,$pre. "_ErrorRate_"  . $rate . "_separately_called_isMeth.txt");

die "$output exists" if (-e $output);
die "cutoff file exists" if(-e $cutoff_file);
if($debug){
	print STDERR "output:\t$output\n";
	print STDERR $cutoff_file, "\n";
}

die "debug: line 52 files are all right" if($debug);
