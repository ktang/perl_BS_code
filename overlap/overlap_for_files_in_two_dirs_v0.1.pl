...
#v0.1
#as in v0.0 original script, the coor type:bed or coor is for both dir;
# for this version, each dir will be assigned a coor type and allowed for gap
...
my $usage = "$0 \n<indir_bench mutants_cared> <coor type:bed or coor> <indir_db> <coor type:bed or coor> <outdir> <gap> \n\n";

die $usage unless (@ARGV == 6);

my $indir_bench = shift or die $usage;
my $coor_type_care = shift or die $usage;
unless (defined $types{$coor_type_care}){
	die "coor type:bed or coor";
}


my $db_dir =  shift or die $usage;
my $coor_type_db = shift or die $usage;
unless (defined $types{$coor_type_db}){
	die "coor type:bed or coor";
}

my $outdir = shift or die "outdir";
my $gap = shift;
die "gap should >=0" unless($gap >= 0);
...
die "wrong bench_dir" unless (-d $indir_bench );
die "wrong db_dir" unless (-d $db_dir );
...
for my $i(0.. ($#files)){
	...
	if ($file =~ /(\S+)\.txt$/) {
		...
		my $output = File::Spec->catfile($outdir, "overlap_gap" . $gap . "bp_" . $pre . ".txt" );
		die if (-e $output);
		...
		if (! $debug) {
			...
			foreach my $db(@dbs){
				...
				my @tmp = get_overlap($input, $coor_type_care, $db_file, $coor_type_db, $self_num, $gap);
...
#my @tmp = get_overlap($input, $coor_type_care, $db_file, $coor_type_db, $self_num);
sub get_overlap{
	#my ($input_sub, $db_file_sub, $coor_type_sub,$self_num_sub) = @_;
	my ($input_sub,$coor_type_care_sub, $db_file_sub, $coor_type_db_sub,$self_num_sub, $gap_sub) = @_;
	...
	while (<DB>) {
		...
		my @a = split "\t";
		my ($chr, $start, $end);
		if ($coor_type_db_sub eq "bed" ) {
			($chr, $start, $end) = @a[0..2];
		}elsif($coor_type_db_sub eq "coor" ){
			if ($a[0] =~ /(\S+):(\d+)-(\d+)/) {
				($chr, $start, $end) = ($1, $2, $3);
			}else{
				die $a[0];
	...
	while (<IN>) {
		...
		my @a = split "\t";
		my ($chr, $start, $end);
		if ($coor_type_care_sub eq "bed" ) {
			($chr, $start, $end) = @a[0..2];
		}elsif($coor_type_care_sub eq "coor" ){
			if ($a[0] =~ /(\S+):(\d+)-(\d+)/) {
				($chr, $start, $end) = ($1, $2, $3);
			}else{
				die $a[0];
		...
		$chr = simple_chr($chr);
		my $flag = 0;
		$start = ( $start - $gap_sub >= 0) ? ($start - $gap_sub):(0);
		$end = $end + $gap_sub;
		for my $i ($start..$end){
			if ( defined $r{$chr}->[$i]) {
				$flag = 1;
