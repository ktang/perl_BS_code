...
foreach my $file( @bed_lists ){
	...
	if($file =~ /(\S+)_coordinate_bed.txt$/){
		...
		$outputs[$i] = File::Spec->catfile($outdir,
			$feature_pres[$i] . "_in_" . $sample_label . "_binNum" . $feature_bin_num . "_flaking" . $flaking_bp . "bp_binNum" . $flaking_bin_num . "_dep" . $dep_cutoff . "_". $postfix . "_notEckerSuggest.txt");
...
#		cal_list($curr_chr, \@input_list,         \@positions, \@depths_temp, \@mCs_temp,  \%total_depths, \%total_mCs, $feature_bin_num, $flaking_bp, $flaking_bin_num );
	...
	for $i (0..$last_index){
		print STDERR "cal $i \n";
		cal_list($curr_chr,  \@{$input_list[$i]}, \@positions, \@depths_temp, \@mCs_temp,  \%{$total_depths[$i]}, \%{$total_mCs[$i]}, $feature_bin_num, $flaking_bp, $flaking_bin_num );
	}
...
sub read_list2{
	...
	print STDERR "read_list2: $file index(line_nunber - 2): $i \n\n";
}
