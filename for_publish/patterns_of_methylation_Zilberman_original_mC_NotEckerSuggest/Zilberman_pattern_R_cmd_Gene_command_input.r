# R --slave --vanilla --args   png_pre   dir < /Users/tang58/Kai_BS/for_publish/10_gene_TE_bin_files_R_cmd_Gene_command_input.r
...
#file_name = paste("ibm2Paper_Gene_length", "_w", width_val, "_h", height_val, "_res", res_val, "_pt", pointsize_val, ".png", sep="")
...
for (i_file_list in 1:length(files_list ) ){
	...
	x_lables = sub("Gene.+_in_(.+)_binNum20_flaking2000bp_binNum20.+.txt", "\\1", files, perl =T);
...
#	axis(1, labels = c("up 2kb", "Gene", "down 2kb"), at = c(11,31,51), tick =F, cex.axis = 1.4, font =2)
			if(i == 3){
...
par(xpd=NA)
################
legend(-280, -0.7,legend= x_lables , col = cols, lwd = 3, bty = "n",  cex = 0.6
,   ncol = length,
