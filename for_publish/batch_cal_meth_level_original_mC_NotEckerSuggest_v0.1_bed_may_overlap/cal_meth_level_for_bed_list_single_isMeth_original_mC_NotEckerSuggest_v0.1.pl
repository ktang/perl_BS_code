...
#v0.1 Nov 18, 2013
# as in v0.0, the bed-like regions will not overlap,
# but in this v0.1 version, the region may be overlapped.
#


#Nov16, 2013 mofied from v0.0 still v0.0
# modified from cal_meth_level_for_bed_list_single_isMeth_original_mC_NotEckerSuggest_v0.0.pl
#cal_meth_level_for_bed_list_single_isMeth_original_mC_NotEckerSuggest_v0.0_backup is tha original
# what I want to modify this time:
# the chr, as in the original script, only chr1, chr2.. chr5 is accepted in the bed_like file
# after this modification, both chr\d and 1..5 are supported.
...
#	open(WT, $isMeth_file_WT_sub) or die;
	...
	while( $l_mut = <MUT>){
		...
		# %{wt/mut}->{1/2}->{CG/CHG/CHH/total}
			...
			my @indexs = split "_",  $pos_ref->{$chr}->[$pos];
