...
my $script =  "/Users/tang58/Kai_BS/for_publish/batch_cal_meth_level_original_mC_NotEckerSuggest/cal_meth_level_for_bed_list_original_mC_NotEckerSuggest_multiple_isMeth.pl";
...
foreach my $file( @files ){
	...
	if($file =~ /(\S+)\.txt$/){
		$out_name = $1 . "_with_meth_original_mC_NotEckerSuggest.txt";
	}else{
		die $file;
	}
