...
#my $usage = "$0 \n<indir> <tar_file> <s1_file> <s2_file> <outdir> <pre> \n\n";
#die $usage unless(@ARGV == 6);
my $usage = "$0 \n<indir> <tar_file> <s1_file> <s2_file> <pre> \n\n";
die $usage unless(@ARGV == 5);

my $indir = shift or die "indir";
my $tar_file = shift or die "tar_file";

my $in_seq1 = shift or die "in_seq1";
my $in_seq2 = shift or die "in_seq2";
...
###	my $in_tar_file = File::Spec->catfile($indir, $tar_file);

###	die unless (-e $in_tar_file);

my $fq_in1 = File::Spec->catfile( $outdir , $in_seq1);
my $fq_in2 = File::Spec->catfile( $outdir , $in_seq2);
...
#my $cmd_tar = "time tar -xzvf $in_tar_file";
#print STDERR $cmd_tar, "\n\n";
if(!$debug){
#	`$cmd_tar`;
	die unless (-e $fq_in1);
	die unless (-e $fq_in2);
...
my $trim_cmd = "time $trim_tool -1 $fq_in1 -2 $fq_in2 -P $outdir/$pre -q $q -L $L -m $m_trim";
...
#PE reads 1,2
my $in_P1 = File::Spec->catfile($outdir, $pre."_reads1.txt");
my $in_P2 = File::Spec->catfile($outdir, $pre."_reads2.txt");

#SE mates 1,2
my $in_S1 = File::Spec->catfile($outdir, $pre."_mates1.txt");
my $in_S2 = File::Spec->catfile($outdir, $pre."_mates2.txt");
...
my $single_list = File::Spec->catfile($outdir, $outpre."_single_list.txt");
die if (-e $single_list);
#my $pair_list   = File::Spec->catfile($indir, $outpre."_pair_list.txt");
my $pair_list   = File::Spec->catfile($outdir, $outpre."_pair_list.txt");
die if (-e $pair_list );
...
my $single_list_nodupl = File::Spec->catfile($outdir, $outpre."_single_list_nodupl.txt");
die if (-e $single_list_nodupl);
#my $pair_list_nodupl   = File::Spec->catfile($indir, $outpre."_pair_list_nodupl.txt");
my $pair_list_nodupl   = File::Spec->catfile($outdir, $outpre."_pair_list_nodupl.txt");
...
my $cmd_acgt_count = "time $acgt_count -r $fas_ref -P $outdir/$outpre -p $pair_list_nodupl -s $single_list_nodupl -B";
