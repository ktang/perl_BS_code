...
my $script = "/Users/tang58/Kai_BS/Jacobsen_Cell_method/extract_common_DMR_vs_diffWT_for_JacobsenCellMethod.pl";
die unless (-e $script);
...
#dj28-1A_At4g21430_vs_colA_CG_hyper_JacobsenCell_Bin100_FDR0.01_sDep4_lDep4_Cnum4.txt
sub get_label{
	my  ($file) = @_;
	#if ($file =~ /(\S+)_vs_/) {
	if ($file =~ /_vs_(\S+)_C[HG]/) {
		return $1;
	}else{
		die $file;
	}
